...
class DrivingConfig(object):
    ...
    env.car_limit = 30      # car limit (for calculate timeouts)
    ...
    car.max_veh_num = 12
    car.safe_control = True

    # reward function
    reward = BaseConfig()
    reward.collision_cost = 3.
    reward.outroad_cost = 2.
    reward.survive_reward = 0.013
    # reward.survive_reward = 0.00
    reward.goal_reward = 3.
    reward.gamma = 0.99

    reward.social_beta_only_collision = False
    reward.social_reward_only_collided = False
    reward.stop_penalty_social = 0.0005

    # ego car (robot car)
    robot = BaseConfig()
    # lstm_attn or lstm_no_attn or lstm_attn_gnn
