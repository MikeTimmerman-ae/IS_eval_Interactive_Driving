...
class Car:
    ...
    def set_obsdistance(self, obs_distance):
        self._obs_distance = obs_distance
    ...
    def setup_render(self, viewer, awr=None):
        ...
        # if not (awr is None):
        #     self.awr = True
        #     # self.awr_geom = rendering.make_circle(np.random.randint(3, 14), res=100, filled=False)
        #     self.awr_geom = rendering.make_circle(self._obs_distance, res=100, filled=False)
        #     self.awr_geom.set_linewidth(2.0)
        #     self.awr_xform = rendering.Transform()
        #     self.awr_geom.set_color(*(0.6, 0.3, 0.3))
        #     self.awr_geom.add_attr(self.awr_xform)
        #     viewer.add_geom(self.awr_geom)
    ...
    def update_render(self, camera_center):
        ...
        if self.awr:
            self.awr_xform.set_translation(*(self.position - camera_center))

    def remove_render(self, viewer):
        viewer.geoms.remove(self.geom)
        viewer.geoms.remove(self.arr_geom)
        if self.awr:
            viewer.geoms.remove(self.awr_geom)
