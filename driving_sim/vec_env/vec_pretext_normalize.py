...
class VecPretextNormalize(VecEnvWrapper):
    ...
    def process_obs(self, O, done):
        ...
        if self.config.pretext.env_name == 'TIntersectionRobustnessSocial-v0':
            return O

        if self.pred_model == 'Xiaobai-train':
            true_label_one_hot = torch.nn.functional.one_hot(O['true_labels'].to(torch.long), num_classes=2)
            O['spatial_edges'][:, :, -2:] = true_label_one_hot
        ...
        elif self.config.pretext.env_name == 'TIntersectionPredictFrontAct-v0':
            for key in self.input_key_list:
                if key == 'dones':
        ...
        elif self.config.pretext.env_name == 'TIntersectionPredictFront-v0' or \
                self.config.pretext.env_name == 'TIntersectionRobustness-v0' or \
                self.config.pretext.env_name == 'TIntersectionRobustnessEgoSocial-v0':
            for key in self.input_key_list:
                if key == 'dones':
                    self.pred_inputs[key].append(torch.from_numpy(done).bool().to(self.device))
                else:
        ...
        if len(self.pred_inputs['pretext_nodes']) == self.pred_period:
            ...
            if len(seq_len) > 0:
                ...
                with torch.no_grad():
                    output = self.pred_model.predict(model_inputs, self.pred_model_hidden_states, seq_len.cpu(), rl=True)

                xiaobai = True
                if xiaobai:
                    # true_label_one_hot = torch.nn.functional.one_hot(O['true_labels'].to(torch.long), num_classes=2)
                    # self.pred_outputs = true_label_one_hot
                    # O['spatial_edges'][:, :, -2:] = true_label_one_hot
                    removed_car_mask = torch.logical_not(O['pretext_masks'])
                    output = torch.nn.functional.one_hot(output.argmax(-1), num_classes=2).float()
                    self.pred_outputs[removed_car_mask] = torch.zeros(*self.pred_outputs[removed_car_mask].size(),
                                                                      device=self.device)
        ...
        if self.config.pretext.env_name == 'TIntersectionRobustness-v0' or \
                self.config.pretext.env_name == 'TIntersectionRobustnessEgoSocial-v0':
            return O
        else:
    ...
    def clear_pred_inputs(self):
        ...
        for key in self.input_key_list:
            # self.pred_inputs[key] = []
            self.pred_inputs[key] = self.pred_inputs[key][1:]

    def clear_pred_inputs_eval(self):
        self.pred_inputs.clear()
        for key in self.input_key_list:
            self.pred_inputs[key] = []