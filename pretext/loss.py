...
class CVAE_loss(object):
    ...
    '''
    def create_bce_mask(self, each_seq_len, device=None):
        # why +1: use a sentinel in the end to handle the case when each_seq_len = 20
        if device:
            mask = torch.zeros(self.batch_size, self.max_seq_len + 1).to(device)  # [1024, 21]
        else:
    ...
    def forward(self, true_act, pred_act, z_mean, z_log_var, each_seq_len=None, train=True):
        if each_seq_len: # mask out the padded sequences from loss calculation
            mask = self.create_bce_mask(each_seq_len, device=true_act.device)
...
class Classifier_loss(CVAE_loss):
    def __init__(self, config, schedule_kl_method=None):
        super(Classifier_loss, self).__init__(config, schedule_kl_method)
        self.loss = torch.nn.CrossEntropyLoss()

    def forward(self, true_act, pred_act, each_seq_len=None, train=True):
        if each_seq_len: # mask out the padded sequences from loss calculation
            mask = self.create_bce_mask(each_seq_len, device=true_act.device)[:, -1]
            ce_loss = self.loss(pred_act[mask], true_act[mask])
        else:
            ce_loss = self.loss(pred_act, true_act)
