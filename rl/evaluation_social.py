...
def evaluate(actor_critic, eval_envs, num_processes, device, config, logging, visualize=False):
    ...
    episode_rewards_env = np.zeros(eval_envs.num_envs)
    ...
    eval_recurrent_hidden_states['rnn'] = torch.zeros(num_processes, edge_num,
                                                       ...
                                                       device=device)
    eval_masks = torch.zeros(num_processes, edge_num, device=device)
    ...
    for k in range(test_size):
        ...
        while not done:
            ...
            ego_action = torch.zeros((eval_envs.num_envs, 1), dtype=torch.int32) + 2
            action_all = torch.cat([ego_action, action], dim=-1)
            obs, rew, done, infos = eval_envs.step(action_all)
